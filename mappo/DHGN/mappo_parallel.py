...
from replay_buffer import ReplayBuffer, BigBuffer
...
class MAPPO:
    def __init__(self, args, batch_size, mini_batch_size, agent_type):
        ...
        self.rnn_input_dim = args.embedding_dim
        self.rnn_hidden_dim = args.rnn_hidden_dim
        self.sn = args.use_spectral_norm
    ...
    def train(self, replay_buffer, total_steps):
        ...
        for index in BatchSampler(SequentialSampler(range(self.batch_size)), self.mini_batch_size, False):
            ...
            actor_embedding_dataset = EmbeddingDataset2(attribute=batch['actor_historical_embedding'], adjacent=batch['p_adj'], is_critic=False, depth=self.args.depth)
            critic_embedding_dataset = EmbeddingDataset2(attribute=batch['critic_historical_embedding'], adjacent=batch['p_adj'], is_critic=True, depth=self.args.depth)
    ...
    def explore_env(self, env, buffer_id, p_num, num_episode, map_info):
        ...
        for k in range(num_episode):
            episode_reward, episode_steps = self.run_episode(env, map_info, p_num=p_num, num_episode=k, idx=buffer_id)  # 5 + i % 11
            exp_reward += episode_reward
            sample_steps += episode_steps
        return p_num, exp_reward / num_episode, self.minibuffer, sample_steps
