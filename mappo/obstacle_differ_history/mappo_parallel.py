...
from mappo.obstacle_differ_history.module.replay_buffer import MiniBuffer
from mappo.obstacle_differ_history.module.normalization import Normalization
...
class GnnExtractor(nn.Module):
    def __init__(self, input_size, middle_size, output_size, n_hops: int = 1, is_sn: bool = False):
        ...
        self.bottleneck = nn.Sequential(
            preproc_layer(output_size + output_size, output_size) if is_sn else nn.Linear(output_size + output_size, output_size),
            nn.ReLU(),
        )
...
class SharedActor(nn.Module):
    ...
    def forward(self, state, adj, hidden_state, last_comm_embedding, mode):
        ...
        assert comm_embedding.shape == last_comm_embedding.shape
...
class MAPPO:
    def __init__(self, args, batch_size, mini_batch_size, agent_type):
        ...
        # self.actor = torch.load('./model/actor.pth').to(self.device)
        # self.critic = torch.load('./model/critic.pth').to(self.device)
    ...
    def run_episode(self, env, map_info, p_num=15, num_episode=0, idx=0):  #
        ...
        actor_last_comm_embedding = torch.zeros(size=(p_num, self.gnn_output_dim), dtype=torch.float32, device=self.device)
        critic_last_comm_embedding = torch.zeros(size=(p_num, self.gnn_output_dim), dtype=torch.float32, device=self.device)
        for episode_step in range(self.args.episode_limit):
            ...
            actor_last_comm_embedding = actor_comm_embedding
            critic_last_comm_embedding = critic_comm_embedding
            if done:
                break
