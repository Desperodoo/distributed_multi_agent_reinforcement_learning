...
class EvaluatorProc(object):
    def __init__(self, args, num_cpus_eval):
        ...
        # print(current_cwd)
        # self.cwd = current_cwd + args.save_cwd
        ...
        # self.recorder_path = f'{self.cwd}/recorder.npy'
    ...
    def run(self, actor_weights, critic_weights, total_step, exp_r, exp_win_rate, exp_collision_rate, logging_tuple):
        ...
        # self.save_or_load_recoder(if_save=True)
        # print(f'| TrainingTime: {time.time() - self.start_time:>7.0f} | SavedDir: {self.cwd}')
    ...
    def evaluate_and_save(self, new_total_step: int, exp_r: float, exp_win_rate: float, exp_collision_rate: float, logging_tuple: tuple):
        ...
        if if_save:
            ...
            return []
            # self.save_training_curve_jpg()
            # torch.save(actor, self.cwd + '/actor_best.pth')  # save policy network in *.pt
...
def evaluate(env, actor, p_num, args):  #
    ...
    actor_last_comm_embedding = torch.zeros(size=(p_num, args.gnn_output_dim), dtype=torch.float32, device=device)
    ...
    for step in range(args.episode_limit):
        ...
        a_n, hidden_state, actor_last_comm_embedding = actor.choose_action(state, adj, hidden_state, actor_last_comm_embedding, deterministic=True)
        # Take a step
